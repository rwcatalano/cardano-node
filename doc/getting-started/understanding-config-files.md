...
* The main difference between `LocalProducers` and `PublicProducers` is that with the former
    you can specify different groups with different valencies. That can be useful to
    inform your node of different targets withing a group to achieve. `LocalProducers`
    is for peers which the node always should have as hot, such as their own block producer.
    `PublicProducers` represent a source of fallback peers, a source of peers to be used if peers
